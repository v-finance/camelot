...
class ItemModelCase(ModelThreadTestCase):

    def setUp( self ):
        super(ItemModelCase, self).setUp()
        settings.setup_model()
        self.app_admin = ApplicationAdmin()
        self.person_admin = self.app_admin.get_related_admin( Person )
        
    def _load_data(self, item_model = None):
    ...
    def _data(self, row, column, item_model=None, role=Qt.EditRole, validate_index=True):
    ...
    def _set_data(self, row, column, value, item_model=None, validate_index=True):
    ...
    def _header_data(self, section, orientation, role):
        return variant_to_py(self.item_model.headerData(section, orientation, role))

    def _flags(self, row, column):
        index = self.item_model.index( row, column )
        return self.item_model.flags(index)

    def _row_count(self, item_model = None):
...
class CollectionProxyCase(ItemModelCase):
    ...
    def test_vertical_header_data(self):
        ...
        for row in range(row_count):
            ...
            self.assertEqual(self._header_data(row, Qt.Vertical, Qt.SizeHintRole), self.item_model.vertical_header_size)
        ...
        for row in range(row_count):
            ...
            self.assertEqual(self._header_data(row, Qt.Vertical, Qt.DisplayRole), str(row+1))
...
class QueryQStandardItemModelCase(
    ...
    def test_single_query(self):
        ...
        proxy = CollectionProxy(self.person_admin)
