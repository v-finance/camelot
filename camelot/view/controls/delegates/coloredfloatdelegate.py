  ...
  def paint(self, painter, option, index):
    ...
    if value >= 0:
      if value == 0:
        ...
        QtGui.QIcon(Icon('tango/16x16/actions/go-up.png').getQPixmap()).paint(painter, option.rect.left(), option.rect.top(), rect.height(), rect.height())
        #QtGui.QApplication.style().drawItemPixmap(painter, rect, 1, icon)
        fontColor.setRgb(0, 255, 0)
    else:
      QtGui.QIcon(Icon('tango/16x16/actions/go-down-red.png').getQPixmap()).paint(painter, option.rect.left(), option.rect.top(), rect.height(), rect.height())
#      QtGui.QApplication.style().drawItemPixmap(painter, rect, 1, icon)
