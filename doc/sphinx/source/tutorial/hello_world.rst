...
The complete application can be tested using nothing more than the :mod:`unittest` module from the standard library.
As an example, create a file :file:`test.py` with this code, in the same directory as :file:`main.py`.

.. literalinclude:: ../../../../test/snippet/hello_world/test.py

The tests can be started from the command line, or from the IDE ::
...
Where to go from here depends on the kind of application you want to build.  Here are some pointers :
  ...
  * Have a look at the various action steps that can be used :ref:`inside the model run method <doc-inside-model-run>`.
    Such as :
      ...
      * :class:`camelot.view.action_steps.print_preview.PrintPreview` to allow the user to preview and print a document
      * :class:`camelot.view.action_steps.select_file.SelectFile` to ask the user to select a file that the application
        will process

  * When building a real application, don't just subclass :class:`Action`, but subclass :class:`camelot.admin.action.application.Application`
    which will take care of the splash screen, set the application name for the OS and others.
